...
(function () {
  ...
  /* Add the IRM mark after standalone IRM references in patient-facing copy */
  const addIrmMarks = () => {
    const root = document.body;
    if (!root) return;

    const skipTags = new Set(['SCRIPT', 'STYLE', 'NOSCRIPT', 'SVG', 'IMG', 'BUTTON', 'TEXTAREA', 'INPUT', 'SELECT', 'TITLE', 'META', 'HEAD']);
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
      acceptNode(node) {
        const parent = node.parentElement;
        if (!parent || skipTags.has(parent.tagName) || parent.closest('.brand, .irm-marked, .nav__links, .mobile')) {
          return NodeFilter.FILTER_REJECT;
        }
        return /\bIRM(?:['’]s)?\b/.test(node.nodeValue) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT;
      }
    });

    const nodes = [];
    while (walker.nextNode()) nodes.push(walker.currentNode);
    const darkInlineLogoContexts = '.footer, .section--cta, .page-card--blue, .nextstep-card--blue, .expert-card--blue, .hero--dark, .filter-tabs--dark';
    const isDarkSurface = element => {
      if (element?.closest(darkInlineLogoContexts)) return true;
      let current = element;
    ...
    nodes.forEach(node => {
      ...
      const parts = node.nodeValue.split(/(\bIRM(?:['’]s)?\b)/g);
      const useInvertedCircle = isDarkSurface(node.parentElement);
      parts.forEach(part => {
        if (/^IRM(?:['’]s)?$/.test(part)) {
          const wrap = document.createElement('span');
          wrap.className = 'irm-marked';
          wrap.append(document.createTextNode(part));
          const img = document.createElement('img');
          img.className = 'irm-inline-logo';
          img.src = useInvertedCircle ? './assets/irm-dna-circle-white.png' : './assets/irm-dna-circle-blue.png';
          img.alt = '';
          img.loading = 'lazy';
          img.decoding = 'async';
          wrap.append(img);
